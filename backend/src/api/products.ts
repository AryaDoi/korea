...
export async function handleCategoryRequest(
  ...
  if (request.method === 'GET') {
    if (params?.id) {
      ...
      if (!category) return errorResponse('Category not found', 404, corsHeaders(env));
...
export async function handleProductRequest(
  ...
  if (request.method === 'GET') {
    if (params?.id) {
      ...
      if (!product) return errorResponse('Product not found', 404, corsHeaders(env));
    ...
    return successResponse({ products, pagination: { page, limit, total, pages: Math.ceil(total / limit) } }, undefined, corsHeaders(env));
  }
