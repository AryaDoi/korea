...
async function loadStatistics() {
  try {
    ...
    document.getElementById('total-revenue').textContent = formatCurrency(stats.data?.total_revenue || 0);
  } catch (error) {
...
async function loadOrders() {
  try {
    ...
    document.getElementById('orders-tbody').innerHTML = '<tr><td colspan="6">Gagal memuat pesanan</td></tr>';
  }
}
...
function setupOrdersSearch() {
  ...
  searchBtn?.addEventListener('click', async () => {
    ...
    try {
      const response = await api.getOrders(searchTerm, 1);
      const orders = response.data?.orders || [];
      const tbody = document.getElementById('orders-tbody');
