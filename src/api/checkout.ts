...
export async function handleCheckoutRequest(
  ...
  if (request.method === 'GET') {
    ...
    if (!session || !session.cart) {
      return successResponse({ items: [], total: 0 }, undefined, corsHeaders(env));
    }
  ...
  if (request.method === 'POST') {
    ...
    if (!validateEmail(body.email)) {
      return errorResponse('Invalid email', 400, corsHeaders(env));
    }
    ...
    if (!created) {
      return errorResponse('Failed to create order', 500, corsHeaders(env));
    }
...
export async function handleCartRequest(
  ...
  if (request.method === 'POST') {
    ...
    if (!body || !body.barang_id || !body.qty) {
      return errorResponse('Missing barang_id or qty', 400, corsHeaders(env));
    }

    const success = await sessionMgr.addToCart(
      sessionId,
      body.barang_id,
      body.qty
    );
    ...
    if (!success) {
      return errorResponse('Failed to add to cart', 500, corsHeaders(env));
    }
  ...
  if (request.method === 'DELETE') {
    ...
    if (!body || !body.barang_id) {
      return errorResponse('Missing barang_id', 400, corsHeaders(env));
    }

    const success = await sessionMgr.removeFromCart(sessionId, body.barang_id);

    if (!success) {
      return errorResponse('Failed to remove from cart', 500, corsHeaders(env));
    }
